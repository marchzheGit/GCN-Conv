...
int main() {
    ...
    // Load x
    sprintf(file_name, "./%s/x.bin", DATASET);
    ...
    // Load weights
    sprintf(file_name, "./%s/weight_conv1.txt", DATASET);
    ...
    // Load edge indices
    sprintf(file_name, "./%s/edge_index.txt", DATASET);
    if (!(ifp = fopen(file_name, "r"))) {
    ...
    timersub(&t1, &t2, &tr);
    printf("Complish GCN calculation step. [%.5f sec elapsed]\n",
        -tr.tv_sec-(double)tr.tv_usec/1000000.0);

    sprintf(file_name, "./%s/result.txt", DATASET);
